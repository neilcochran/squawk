...
function expandWorkspacePattern(pattern) {
  ...
  if (!pattern.endsWith('/*') || pattern.slice(0, -2).includes('*')) {
    ...
    // eslint-disable-next-line n/no-process-exit -- malformed workspaces pattern in the root manifest; this runs inside a flatMap helper with no return path to the entry point.
    process.exit(1);
  }
...
/**
 * Checks that every publishable workspace has build output in dist/.
 *
 * @returns The exit code: 0 when all of them do, 1 otherwise.
 */
function main() {
