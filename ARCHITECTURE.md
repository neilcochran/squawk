...
Toggling gates registration only. Snapshots still load at module import, so a trimmed catalog does not mean a smaller or faster process, and a group that reads another group's data keeps working when that other group is disabled.
