# @squawk/icao-registry

Resolves a 24-bit ICAO hex address to its aircraft registration and info using the FAA ReleasableAircraft database. Supports live FAA lookups, a bundled offline snapshot, and custom datasets for non-US registries.

**[Documentation](https://neilcochran.github.io/squawk/modules/_squawk_icao-registry.html)**

## Installation

```bash
npm install @squawk/icao-registry
```

> This package is under active development. See the [documentation](https://neilcochran.github.io/squawk/modules/_squawk_icao-registry.html) for current API status.
