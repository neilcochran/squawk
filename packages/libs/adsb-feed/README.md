...
- `isEmergencyAircraft(aircraft)` - whether an `Aircraft` is in an emergency: an emergency squawk, a declared emergency state, or an active ACAS/TCAS Resolution Advisory. Any one is sufficient, since which of them a feed can see depends on its source (see [Field population by source](#field-population-by-source)). Built from `isEmergencySquawk(squawk)` (7500, 7600, or 7700 - the `EMERGENCY_SQUAWKS` set) and `isDeclaredEmergencyState(emergencyState)` (any `EmergencyState` other than `'none'` and `'reserved'`), which are exported too. All are pure, and available from the browser entry point as well.
