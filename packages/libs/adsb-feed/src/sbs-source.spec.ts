...
describe('connection state', () => {
  ...
  it('dispatches connection:connect and updates getConnectionState once the socket connects', async () => {
    const feed = createSbsAircraftFeed({ host: '127.0.0.1', port });
  ...
  it('dispatches connection:disconnect when the server closes the socket', async () => {
    const feed = createSbsAircraftFeed({ host: '127.0.0.1', port, reconnectDelayMs: 20 });
