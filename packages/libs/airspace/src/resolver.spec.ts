...
describe('createAirspaceResolver with real data', () => {
  describe('LAX Class B', () => {
    ...
    it('returns CLASS_B features for a point at LAX at low altitude', () => {
      ...
      assert(classB.length > 0, 'expected at least one CLASS_B feature');
      assert(
    ...
    it('returns the SFC ring at ground level', () => {
      ...
      assert(sfcRing, 'expected a CLASS_B ring with SFC floor');
    });

    it('returns no CLASS_B above 10000 ft (LAX ceiling)', () => {
      const results = resolve_.query({ lat: 33.9425, lon: -118.4081, altitudeFt: 12000 });
      const classB = results.filter((f) => f.type === 'CLASS_B');
      expect(classB.length, 'no CLASS_B expected above LAX ceiling').toBe(0);
    });
  });
  ...
  describe('ORD Class B', () => {
    ...
    it('returns CLASS_B for a point at ORD', () => {
      ...
      const classB = results.filter((f) => f.type === 'CLASS_B');
      assert(classB.length > 0, 'expected CLASS_B at ORD');
      assert(
  ...
  describe('Class E only (rural CONUS)', () => {
    ...
    it('returns only Class E at low altitude in open terrain', () => {
      ...
      assert(results.length > 0, 'expected Class E coverage in rural Kansas');
  ...
  describe('no controlled airspace', () => {
    it('returns no Class B/C/D/E or SUA features over the mid-Atlantic', () => {
      ...
      const nonArtcc = results.filter((f) => f.type !== 'ARTCC');
      expect(nonArtcc.length, 'expected no Class B/C/D/E or SUA over mid-Atlantic').toBe(0);
    });
    ...
    it('returns no Class B/C/D/E or SUA features over the Pacific off the California coast', () => {
      ...
      const nonArtcc = results.filter((f) => f.type !== 'ARTCC');
      expect(nonArtcc.length, 'expected no Class B/C/D/E or SUA over the Pacific').toBe(0);
    });
  });
  ...
  describe('Class C', () => {
    ...
    it('returns CLASS_C for a point at Louisville SDF', () => {
      ...
      assert(classC.length > 0, 'expected CLASS_C at SDF');
      assert(
  ...
  describe('Class D', () => {
    ...
    it('returns CLASS_D for a point at Santa Fe SAF', () => {
      ...
      assert(classD.length > 0, 'expected CLASS_D at SAF');
      assert(
  ...
  describe('Class E2', () => {
    ...
    it('returns CLASS_E2 for a point at Gainesville GNV', () => {
      ...
      assert(classE2.length > 0, 'expected CLASS_E2 at GNV');
      assert(
  ...
  describe('Class E5', () => {
    ...
    it('returns CLASS_E5 for a point in open terrain', () => {
      ...
      assert(classE5.length > 0, 'expected CLASS_E5 in open terrain');
    });
  });
  ...
  describe('SUA - Restricted area', () => {
    ...
    it('returns RESTRICTED for a point inside R-2508', () => {
      ...
      assert(restricted.length > 0, 'expected RESTRICTED in R-2508 area');
    });
  });
  ...
  describe('SUA - Prohibited area', () => {
    ...
    it('returns PROHIBITED for a point inside P-56 DC', () => {
      ...
      assert(prohibited.length > 0, 'expected PROHIBITED in P-56 area');
    });
  });
  ...
  describe('SUA - MOA', () => {
    ...
    it('returns MOA for a point inside a known MOA', () => {
      ...
      assert(moas.length > 0, 'expected MOA in ADA EAST area');
    });
  });
  ...
  describe('SUA - Warning area', () => {
    ...
    it('returns WARNING for a point inside a coastal warning area', () => {
      ...
      assert(warnings.length > 0, 'expected WARNING area off SE coast');
    });
  });
  ...
  describe('SUA - Alert area', () => {
    it('at least one ALERT area is queryable in the dataset', () => {
      ...
      if (alerts.length === 0) {
        ...
        assert(hasAlerts, 'ALERT type should exist in the dataset');
      }
    });
  });
  ...
  describe('overlapping airspace', () => {
    it('returns multiple feature types for a point in overlapping airspace', () => {
      ...
      assert(types.size >= 2, `expected multiple types, got: ${[...types].join(', ')}`);
    });
  });
  ...
  describe('returned feature properties', () => {
    it('includes all expected AirspaceFeature fields', () => {
      const results = resolve_.query({ lat: 33.9425, lon: -118.4081, altitudeFt: 3000 });
      assert(results.length > 0, 'expected results at LAX');
      const feature = results[0]!;
      expect(typeof feature.type).toBe('string');
      expect(typeof feature.name).toBe('string');
      expect(typeof feature.identifier).toBe('string');
      assert(feature.floor && typeof feature.floor.valueFt === 'number');
      assert(feature.floor && typeof feature.floor.reference === 'string');
      assert(feature.ceiling && typeof feature.ceiling.valueFt === 'number');
      assert(feature.ceiling && typeof feature.ceiling.reference === 'string');
      assert(feature.boundary && feature.boundary.type === 'Polygon');
      // artccStratum is required on AirspaceFeature; non-ARTCC features carry null.
      assert('artccStratum' in feature, 'artccStratum field must be present');
      expect(feature.artccStratum, 'non-ARTCC feature should have null stratum').toBe(null);
    });
    ...
    it('includes a non-null artccStratum on ARTCC features', () => {
      ...
      assert(ny.length > 0, 'expected ZNY features');
      for (const feature of ny) {
        assert(feature.artccStratum !== null, 'ARTCC feature stratum must not be null');
        assert(validStrata.has(feature.artccStratum), `unexpected stratum ${feature.artccStratum}`);
      }
    });
  });
});
...
describe('type filter', () => {
  it('returns only requested types when types filter is provided', () => {
    ...
    assert(results.length > 0, 'expected CLASS_B results at LAX');
    assert(
  ...
  it('returns no results when filtering for a type not present at location', () => {
    ...
    expect(results.length, 'no MOAs expected at LAX').toBe(0);
  });
  ...
  it('supports filtering for multiple types', () => {
    ...
    const types = new Set(results.map((f) => f.type));
    assert(types.has('PROHIBITED'), 'expected PROHIBITED');
    assert(types.has('CLASS_B'), 'expected CLASS_B');
  });
  ...
  it('filters Class E subtypes independently', () => {
    ...
    expect(e2Only.length, 'no CLASS_E2 expected in rural Kansas').toBe(0);
    ...
    assert(e5Only.length > 0, 'expected CLASS_E5 in rural Kansas');
    assert(
  ...
  it('returns all types when types filter is omitted', () => {
    ...
    const results = resolve_.query({ lat: 38.891, lon: -77.03, altitudeFt: 1000 });
    const types = new Set(results.map((f) => f.type));
    assert(types.size >= 2, 'expected multiple types without filter');
  });
});
...
describe('byAirport', () => {
  it('returns every Class B sector associated with JFK', () => {
    ...
    assert(classB.length > 1, 'JFK Class B is encoded as multiple sectors');
    assert(
  ...
  it('includes the full boundary polygon on each returned feature', () => {
    ...
    assert(features.length > 0, 'expected LAX features');
    for (const feature of features) {
      expect(feature.boundary.type).toBe('Polygon');
      const ring = feature.boundary.coordinates[0];
      assert(ring && ring.length >= 4, 'boundary ring must be present and closed');
    }
  });
  ...
  it('filters by type when the types set is provided', () => {
    ...
    assert(onlyClassB.every((f) => f.type === 'CLASS_B'));
    assert(onlyClassB.length <= all.length);
  });
  ...
  it('does not match ICAO-prefixed codes (use the FAA identifier)', () => {
    ...
    expect(features.length, 'airspace features are keyed by FAA ID, not ICAO').toBe(0);
  });
...
describe('byArtcc', () => {
  it('returns LOW and HIGH features for a CONUS center', () => {
    ...
    assert(features.length >= 2, 'expected at least LOW and HIGH for ZNY');
    assert(
    ...
    assert(strata.has('LOW'));
    assert(strata.has('HIGH'));
  });

  it('filters by stratum when provided', () => {
    const features = resolve_.byArtcc('ZBW', 'HIGH');
    assert(features.length > 0, 'expected at least one ZBW HIGH feature');
    assert(
  ...
  it('returns an empty array for an unknown identifier', () => {
    expect(resolve_.byArtcc('ZZZ').length).toBe(0);
  });
  ...
  it('returns multiple features for a stratum that has been split or has multiple shapes', () => {
    ...
    for (const feature of zakCta) {
      ...
      for (const coord of ring) {
        const lon = coord[0]!;
        assert(lon >= -180 && lon <= 180, `lon ${lon} out of range for ZAK CTA`);
      }
    }
  });
});
...
describe('ARTCC position queries', () => {
  ...
  it('does not return ZAK for a CONUS position (no false positive from antimeridian splits)', () => {
    ...
    expect(zak.length, 'ZAK should not match a CONUS position').toBe(0);
  });
  ...
  it('returns only ARTCC features when the types filter is restricted to ARTCC', () => {
    ...
    assert(results.length > 0, 'expected at least one ARTCC at rural Kansas');
    assert(
