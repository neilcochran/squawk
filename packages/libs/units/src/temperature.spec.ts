...
describe('temperature conversions', () => {
  ...
  describe('fahrenheitToCelsius', () => {
    ...
    it('is the inverse of celsiusToFahrenheit', () => {
      assert(close(fahrenheitToCelsius(celsiusToFahrenheit(25)), 25));
    });
  });
  ...
  describe('celsiusToKelvin', () => {
    ...
    it('converts ISA sea-level standard 15 C to 288.15 K', () => {
      expect(celsiusToKelvin(15)).toBe(288.15);
    });
  });
  ...
  describe('kelvinToCelsius', () => {
    ...
    it('is the inverse of celsiusToKelvin', () => {
      assert(close(kelvinToCelsius(celsiusToKelvin(-56.5)), -56.5));
    });
  });

  describe('fahrenheitToKelvin', () => {
    it('converts 32 F to 273.15 K', () => {
      assert(close(fahrenheitToKelvin(32), 273.15));
    });
    it('converts -40 F to 233.15 K', () => {
      assert(close(fahrenheitToKelvin(-40), 233.15));
    });
  });

  describe('kelvinToFahrenheit', () => {
    it('converts 273.15 K to 32 F', () => {
      assert(close(kelvinToFahrenheit(273.15), 32));
    });
    it('is the inverse of fahrenheitToKelvin', () => {
      assert(close(kelvinToFahrenheit(fahrenheitToKelvin(59)), 59));
    });
  });

  describe('fractional temperature precision', () => {
    it('converts 20.5 C to 68.9 F', () => {
      assert(close(celsiusToFahrenheit(20.5), 68.9, 0.01));
    });
    it('converts 98.6 F to ~37 C', () => {
      assert(close(fahrenheitToCelsius(98.6), 37, 0.01));
    });
    it('converts 25.5 C to 298.65 K', () => {
      assert(close(celsiusToKelvin(25.5), 298.65, 0.01));
    });
    it('converts -0.5 C to 272.65 K', () => {
      assert(close(celsiusToKelvin(-0.5), 272.65, 0.01));
    });
  });

  describe('zero and near-absolute-zero', () => {
    it('converts exactly 0 C to 0.00 K offset', () => {
      expect(celsiusToKelvin(0) - 273.15).toBe(0);
    });
  ...
  describe('reference values (immutable anchors)', () => {
    ...
    it('Absolute zero: -273.15°C = -459.67°F = 0 K', () => {
      assert(close(celsiusToFahrenheit(-273.15), -459.67, 0.01));
      expect(kelvinToCelsius(0)).toBe(-273.15);
    });
  });
  ...
  describe('real-world aviation scenarios', () => {
    it('FL350 ISA tropopause -56.5°C = -69.7°F', () => {
      assert(close(celsiusToFahrenheit(-56.5), -69.7, 0.1));
    });
    it('FL180 ISA temperature -20.66°C = -5.19°F', () => {
      assert(close(celsiusToFahrenheit(-20.66), -5.19, 0.1));
    });
    it('high altitude extreme cold -40°C = -40°F (crossover)', () => {
      expect(celsiusToFahrenheit(-40)).toBe(-40);
    });
    it('cabin pressure cold scenario -20°C = -4°F', () => {
      assert(close(celsiusToFahrenheit(-20), -4, 0.1));
    });
  });
  ...
  describe('critical physics constraint: Temperature >= Absolute Zero', () => {
    it('Absolute zero is -273.15°C = 0 K = -459.67°F', () => {
      expect(celsiusToKelvin(-273.15)).toBe(0);
      assert(close(celsiusToFahrenheit(-273.15), -459.67, 0.1));
    });
    it('Temperature conversions work correctly at near-absolute-zero', () => {
      ...
      assert(tempK > 0, `Kelvin conversion of ${tempC}°C should be > 0`);
      assert(tempK < 1, `Kelvin conversion of ${tempC}°C should be < 1 K`);
    });
    it('Extreme cold (outside atmosphere, liquid oxygen range): -183°C', () => {
      ...
      assert(tempK > 0, `${tempC}°C should convert to positive Kelvin`);
      assert(tempK < 273.15, `${tempC}°C should be well below freezing in Kelvin`);
      assert(tempF < 0, `${tempC}°C should be below freezing in Fahrenheit`);
    });
    it('Physical law: No temperature below -273.15°C exists in nature', () => {
      ...
      expect(absoluteZeroK, 'Absolute zero should convert to 0 K').toBe(0);
      ...
      for (const temp of validTemps) {
        assert(temp >= -273.15, `Test temperature ${temp}°C should be >= absolute zero`);
        const k = celsiusToKelvin(temp);
        assert(k >= 0, `Kelvin result (${k}) for ${temp}°C should be >= 0`);
      }
    });
  });
});
