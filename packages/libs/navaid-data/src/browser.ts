...
/**
 * Options for {@link loadUsBundledNavaids}.
 */
...
/**
 * Asynchronously loads, decompresses, and parses the bundled navaid
 * dataset. Returns the same `NavaidDataset` shape as the Node entry
 * point exports as `usBundledNavaids`.
 *
 ...
 * ```
 */
export async function loadUsBundledNavaids(
  options?: LoadNavaidDatasetOptions,
): Promise<NavaidDataset> {
  const url = options?.url ?? new URL('../data/navaids.json.gz', import.meta.url);
  const fetchImpl = options?.fetch ?? globalThis.fetch;

  const res = await fetchImpl(url);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch navaid dataset from ${String(url)}: ${res.status} ${res.statusText}`,
    );
  }
  if (res.body === null) {
