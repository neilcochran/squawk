...
Pre-processed snapshot of US instrument procedure data from the **2026-09-03** FAA CIFP
...
Importing the package decompresses and parses the whole procedure snapshot. When all
...
console.log(usBundledProceduresProperties.cifpCycleDate); // YYYY-MM-DD
console.log(usBundledProceduresProperties.recordCount);
```

The values are identical to `usBundledProcedures.properties`; the difference is only
