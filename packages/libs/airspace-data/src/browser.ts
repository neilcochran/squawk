 ...
 * parses the bundled `data/airspace.geojson.gz` snapshot using Web Streams
...
import type { AirspaceDataset } from './node.js';

/**
 * Options for {@link loadUsBundledAirspace}.
 */
 ...
 * ```
 */
export async function loadUsBundledAirspace(
  options?: LoadAirspaceDatasetOptions,
): Promise<AirspaceDataset> {
  const url = options?.url ?? new URL('../data/airspace.geojson.gz', import.meta.url);
  const fetchImpl = options?.fetch ?? globalThis.fetch;

  const res = await fetchImpl(url);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch airspace dataset from ${String(url)}: ${res.status} ${res.statusText}`,
    );
  }
  if (res.body === null) {
  ...
  const dataset: AirspaceDataset = JSON.parse(text);
