...
describe('standardRateBankAngle', () => {
  it('returns a reasonable bank angle for typical GA speed', () => {
    ...
    assert.ok(bank > 14 && bank < 18, `expected 14-18 deg, got ${bank}`);
  });
  ...
  it('returns roughly 25 degrees at 170 kt', () => {
    ...
    assert.ok(bank > 22 && bank < 28, `expected 22-28 deg, got ${bank}`);
  });
});
...
describe('turnRadius', () => {
  ...
  it('decreases with steeper bank', () => {
    ...
    assert.ok(steep < shallow, `expected steep (${steep}) < shallow (${shallow})`);
  });
...
describe('timeToTurn', () => {
  it('computes time for a standard rate 360-degree turn', () => {
    ...
    assert.ok(close(t, 120, 0.01), `expected 120 sec, got ${t}`);
  });

  it('computes time for a 90-degree turn', () => {
    const t = turn.timeToTurn(90);
    assert.ok(close(t, 30, 0.01), `expected 30 sec, got ${t}`);
  });
...
describe('loadFactor', () => {
  it('returns 1g at zero bank', () => {
    assert.ok(close(turn.loadFactor(0), 1, 0.001));
  });

  it('returns 2g at 60 degrees bank', () => {
    assert.ok(close(turn.loadFactor(60), 2, 0.001), `expected 2g, got ${turn.loadFactor(60)}`);
  });

  it('returns approximately 1.41g at 45 degrees bank', () => {
    const lf = turn.loadFactor(45);
    assert.ok(close(lf, Math.SQRT2, 0.001), `expected ~1.414, got ${lf}`);
  });
