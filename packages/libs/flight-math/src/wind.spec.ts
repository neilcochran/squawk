...
describe('solveWindTriangle', () => {
  it('returns zero WCA and full groundspeed with no wind', () => {
    ...
    assert(close(result.groundSpeedKt, 150, 0.1), `expected GS ~150, got ${result.groundSpeedKt}`);
    assert(
  ...
  it('reduces groundspeed with a direct headwind', () => {
    ...
    assert(close(result.groundSpeedKt, 100, 1), `expected GS ~100, got ${result.groundSpeedKt}`);
    assert(
  ...
  it('increases groundspeed with a direct tailwind', () => {
    ...
    assert(close(result.groundSpeedKt, 140, 1), `expected GS ~140, got ${result.groundSpeedKt}`);
  });
  ...
  it('handles wind stronger than TAS by clamping WCA', () => {
    ...
    assert(typeof result.groundSpeedKt === 'number', 'should return a numeric groundspeed');
    assert(typeof result.windCorrectionAngleDeg === 'number', 'should return a numeric WCA');
  });
...
describe('headwindCrosswind', () => {
  it('returns full headwind for a direct headwind', () => {
    ...
    assert(close(result.headwindKt, 20, 0.1), `expected headwind ~20, got ${result.headwindKt}`);
    assert(close(result.crosswindKt, 0, 0.1), `expected crosswind ~0, got ${result.crosswindKt}`);
  });
  ...
  it('returns full tailwind for a direct tailwind', () => {
    ...
    assert(close(result.headwindKt, -20, 0.1), `expected tailwind ~-20, got ${result.headwindKt}`);
    assert(close(result.crosswindKt, 0, 0.1), `expected crosswind ~0, got ${result.crosswindKt}`);
  });
  ...
  it('returns positive crosswind from the right', () => {
    ...
    assert(close(result.headwindKt, 0, 0.1), `expected headwind ~0, got ${result.headwindKt}`);
    assert(close(result.crosswindKt, 15, 0.1), `expected crosswind ~15, got ${result.crosswindKt}`);
  });
  ...
  it('resolves a quartering headwind correctly', () => {
    ...
    assert(close(result.crosswindKt, 10, 0.1), `expected crosswind ~10, got ${result.crosswindKt}`);
  });
...
describe('findWind', () => {
  it('returns zero wind when ground track matches air vector', () => {
    ...
    assert(close(result.speedKt, 0, 0.1), `expected wind speed ~0, got ${result.speedKt}`);
  });
  ...
  it('detects a headwind when GS is less than TAS on the same heading', () => {
    ...
    assert(close(result.speedKt, 20, 0.5), `expected wind speed ~20, got ${result.speedKt}`);
    assert(
  ...
  it('detects a crosswind when heading and track diverge', () => {
    ...
    assert(result.speedKt > 0, `expected nonzero wind speed, got ${result.speedKt}`);
...
describe('crosswindComponent', () => {
  ...
  it('returns the absolute value regardless of side', () => {
    ...
    assert(close(xwRight, xwLeft, 0.01), `expected same magnitude, got ${xwRight} vs ${xwLeft}`);
  });
  ...
  it('resolves a quartering wind correctly', () => {
    ...
    assert(close(xw, 10, 0.1), `expected ~10, got ${xw}`);
  });
});
