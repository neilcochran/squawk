...
describe('fuelRequired', () => {
  it('computes fuel for a known leg', () => {
    ...
    assert(close(fuel, 25, 0.001), `expected 25, got ${fuel}`);
  });

  it('returns zero for zero distance', () => {
    assert(close(planning.fuelRequired(0, 120, 10), 0, 0.001));
  });

  it('scales linearly with distance', () => {
    const short = planning.fuelRequired(100, 120, 10);
    const long = planning.fuelRequired(200, 120, 10);
    assert(close(long, short * 2, 0.001), `expected ${short * 2}, got ${long}`);
  });
  ...
  it('scales linearly with burn rate', () => {
    ...
    assert(close(highBurn, lowBurn * 2, 0.001), `expected ${lowBurn * 2}, got ${highBurn}`);
  });
  ...
  it('increases when ground speed decreases (more time aloft)', () => {
    ...
    assert(slow > fast, `expected slow GS (${slow}) > fast GS (${fast})`);
  });
});
...
describe('endurance', () => {
  it('computes endurance for known values', () => {
    ...
    assert(close(hrs, 5, 0.001), `expected 5, got ${hrs}`);
  });

  it('returns zero for zero fuel', () => {
    assert(close(planning.endurance(0, 10), 0, 0.001));
  });

  it('doubles when fuel doubles', () => {
    const half = planning.endurance(25, 10);
    const full = planning.endurance(50, 10);
    assert(close(full, half * 2, 0.001), `expected ${half * 2}, got ${full}`);
  });
});
...
describe('enduranceDistanceNm', () => {
  it('computes range for known values', () => {
    ...
    assert(close(range, 600, 0.001), `expected 600, got ${range}`);
  });

  it('returns zero for zero fuel', () => {
    assert(close(planning.enduranceDistanceNm(0, 10, 120), 0, 0.001));
  });
  ...
  it('equals endurance multiplied by ground speed', () => {
    ...
    assert(close(range, expected, 0.001), `expected ${expected}, got ${range}`);
  });
});
...
describe('pointOfNoReturn', () => {
  it('computes PNR with no wind (symmetric ground speed)', () => {
    ...
    assert(close(pnr.distanceNm, 300, 0.001), `expected 300 nm, got ${pnr.distanceNm}`);
    assert(close(pnr.timeHrs, 2.5, 0.001), `expected 2.5 hrs, got ${pnr.timeHrs}`);
  });
  ...
  it('returns zero distance and time for zero fuel', () => {
    ...
    assert(close(pnr.distanceNm, 0, 0.001), `expected 0, got ${pnr.distanceNm}`);
    assert(close(pnr.timeHrs, 0, 0.001), `expected 0, got ${pnr.timeHrs}`);
  });
...
describe('equalTimePoint', () => {
  it('returns midpoint with no wind (symmetric ground speed)', () => {
    ...
    assert(close(etp.distanceNm, 250, 0.001), `expected 250 nm, got ${etp.distanceNm}`);
    assert(close(etp.timeHrs, 250 / 120, 0.001), `expected ${250 / 120} hrs, got ${etp.timeHrs}`);
  });
  ...
  it('shifts ETP toward destination with headwind (faster return)', () => {
    ...
    assert(close(etp.distanceNm, 312.5, 0.001), `expected 312.5 nm, got ${etp.distanceNm}`);
    assert(
      etp.distanceNm > 250,
  ...
  it('shifts ETP toward departure with tailwind (slower return)', () => {
    ...
    assert(close(etp.distanceNm, 187.5, 0.001), `expected 187.5 nm, got ${etp.distanceNm}`);
    assert(
      etp.distanceNm < 250,
  ...
  it('returns zero for zero total distance', () => {
    ...
    assert(close(etp.distanceNm, 0, 0.001), `expected 0, got ${etp.distanceNm}`);
    assert(close(etp.timeHrs, 0, 0.001), `expected 0, got ${etp.timeHrs}`);
  });
