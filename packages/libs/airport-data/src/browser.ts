 ...
 * parses the bundled `data/airports.json.gz` snapshot using Web Streams
...
/**
 * Options for {@link loadUsBundledAirports}.
 */
...
/**
 * Asynchronously loads, decompresses, and parses the bundled airport
 * dataset. Returns the same `AirportDataset` shape as the Node entry
 * point exports as `usBundledAirports`.
 *
 ...
 * ```
 */
export async function loadUsBundledAirports(
  options?: LoadAirportDatasetOptions,
): Promise<AirportDataset> {
  const url = options?.url ?? new URL('../data/airports.json.gz', import.meta.url);
  const fetchImpl = options?.fetch ?? globalThis.fetch;

  const res = await fetchImpl(url);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch airport dataset from ${String(url)}: ${res.status} ${res.statusText}`,
    );
  }
  if (res.body === null) {
