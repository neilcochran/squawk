...
describe('densityAltitude', () => {
  ...
  it('combines non-standard pressure and temperature effects', () => {
    ...
    assert.ok(da > daLowPressOnly, `expected combined (${da}) > pressure-only (${daLowPressOnly})`);
    assert.ok(da > daHotOnly, `expected combined (${da}) > temp-only (${daHotOnly})`);
  });
});
...
describe('trueAltitude', () => {
  ...
  it('returns indicated altitude under ISA temp with non-standard pressure', () => {
    ...
    assert.ok(close(ta, indicated, 5), `expected ~${indicated}, got ${ta}`);
  });
  ...
  it('applies correction only above station when stationElevationFt is provided', () => {
    ...
    assert.ok(taStation < taFull, `expected station-corrected (${taStation}) < full (${taFull})`);
    assert.ok(taStation > 10000, `expected station-corrected (${taStation}) > indicated (10000)`);
  });
