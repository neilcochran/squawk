...
describe('solveWindTriangle', () => {
  it('returns zero WCA and full groundspeed with no wind', () => {
    ...
    assert.ok(close(result.windCorrectionAngleDeg, 0, 0.01), `expected WCA ~0, got ${result.windCorrectionAngleDeg}`);
    assert.ok(close(result.groundspeedKt, 150, 0.1), `expected GS ~150, got ${result.groundspeedKt}`);
    assert.ok(close(result.trueHeadingDeg, 360 % 360, 0.01) || close(result.trueHeadingDeg, 360, 0.01),
  ...
  it('computes positive WCA for wind from the right', () => {
    ...
    assert.ok(result.windCorrectionAngleDeg > 0, `expected positive WCA, got ${result.windCorrectionAngleDeg}`);
    assert.ok(result.trueHeadingDeg > 0 && result.trueHeadingDeg < 90,
  ...
  it('computes negative WCA for wind from the left', () => {
    ...
    assert.ok(result.windCorrectionAngleDeg < 0, `expected negative WCA, got ${result.windCorrectionAngleDeg}`);
  });
  ...
  it('reduces groundspeed with a direct headwind', () => {
    ...
    assert.ok(close(result.windCorrectionAngleDeg, 0, 0.5), `expected WCA ~0, got ${result.windCorrectionAngleDeg}`);
  });
...
describe('headwindCrosswind', () => {
  it('returns full headwind for a direct headwind', () => {
    ...
    assert.ok(close(result.crosswindKt, 0, 0.1), `expected crosswind ~0, got ${result.crosswindKt}`);
  });
  ...
  it('returns full tailwind for a direct tailwind', () => {
    ...
    assert.ok(close(result.headwindKt, -20, 0.1), `expected tailwind ~-20, got ${result.headwindKt}`);
    assert.ok(close(result.crosswindKt, 0, 0.1), `expected crosswind ~0, got ${result.crosswindKt}`);
  });
  ...
  it('returns positive crosswind from the right', () => {
    ...
    assert.ok(close(result.crosswindKt, 15, 0.1), `expected crosswind ~15, got ${result.crosswindKt}`);
  });
  ...
  it('resolves a quartering headwind correctly', () => {
    ...
    assert.ok(close(result.headwindKt, 17.32, 0.1), `expected headwind ~17.32, got ${result.headwindKt}`);
    assert.ok(close(result.crosswindKt, 10, 0.1), `expected crosswind ~10, got ${result.crosswindKt}`);
  });
});
...
describe('findWind', () => {
  ...
  it('detects a headwind when GS is less than TAS on the same heading', () => {
    ...
    assert.ok(close(result.directionDeg, 360, 1) || close(result.directionDeg, 0, 1),
  ...
  it('detects a tailwind when GS exceeds TAS on the same heading', () => {
    ...
    assert.ok(close(result.speedKt, 20, 0.5), `expected wind speed ~20, got ${result.speedKt}`);
    assert.ok(close(result.directionDeg, 180, 1), `expected wind from ~180, got ${result.directionDeg}`);
  });
  ...
  it('round-trips with solveWindTriangle', () => {
    ...
    assert.ok(close(recovered.speedKt, knownWind.speedKt, 0.5),
      `expected speed ~${knownWind.speedKt}, got ${recovered.speedKt}`);
    assert.ok(close(recovered.directionDeg, knownWind.directionDeg, 1),
