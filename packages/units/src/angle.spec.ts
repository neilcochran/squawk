...
describe('angle conversions', () => {
  describe('degreesToRadians', () => {
    ...
    it('converts 180 degrees to pi radians', () => {
      assert.ok(close(degreesToRadians(180), Math.PI, 0.000001));
    });
    it('converts 360 degrees to 2*pi radians', () => {
      assert.ok(close(degreesToRadians(360), 2 * Math.PI, 0.000001));
    });
    it('converts 90 degrees to pi/2 radians', () => {
      assert.ok(close(degreesToRadians(90), Math.PI / 2, 0.000001));
    });
    it('converts 45 degrees to pi/4 radians', () => {
      assert.ok(close(degreesToRadians(45), Math.PI / 4, 0.000001));
    });
  });

  describe('radiansToDegrees', () => {
    it('converts 0 radians to 0 degrees', () => {
      assert.equal(radiansToDegrees(0), 0);
    });
    it('converts pi radians to 180 degrees', () => {
      assert.ok(close(radiansToDegrees(Math.PI), 180, 0.000001));
    });
    it('converts 2*pi radians to 360 degrees', () => {
      assert.ok(close(radiansToDegrees(2 * Math.PI), 360, 0.000001));
    });
    it('converts pi/2 radians to 90 degrees', () => {
      assert.ok(close(radiansToDegrees(Math.PI / 2), 90, 0.000001));
    });
  });

  describe('round-trip', () => {
    it('degreesToRadians then radiansToDegrees returns original value', () => {
      assert.ok(close(radiansToDegrees(degreesToRadians(270)), 270, 0.000001));
    });
