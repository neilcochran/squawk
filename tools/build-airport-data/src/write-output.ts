import { mkdir, writeFile } from 'node:fs/promises';
import { dirname } from 'node:path';
import { gzipSync } from 'node:zlib';

import { updateReadmeDate, writeDatasetMeta } from '@squawk/build-shared';
import type { Airport } from '@squawk/types';

/**
 * Shape of the bundled JSON output file: dataset metadata plus the array
 * of full Airport records. Matches the wire format consumed by
 * `@squawk/airport-data`.
 */
interface BundledOutput {
  /** Dataset metadata. */
  meta: {
    /** ISO 8601 timestamp of when the dataset was generated. */
    generatedAt: string;
    /** NASR cycle effective date. */
    nasrCycleDate: string;
    /** Total number of airport records in the dataset. */
    recordCount: number;
  };
  /** Airport records. */
  records: Airport[];
}

/**
 * Writes Airport records to a gzipped JSON file at the given path.
 * Creates the output directory if it does not exist.
 *
 * @param airports - Airport records to serialize.
 * @param nasrCycleDate - NASR cycle effective date string.
 * @param outputPath - Absolute path to write the output .json.gz file.
 */
export async function writeOutput(
  airports: Airport[],
  nasrCycleDate: string,
  outputPath: string,
): Promise<void> {
  await mkdir(dirname(outputPath), { recursive: true });

  const output: BundledOutput = {
    meta: {
      generatedAt: new Date().toISOString(),
      nasrCycleDate,
      recordCount: airports.length,
    },
    records: airports,
  };

  const json = JSON.stringify(output);
  const compressed = gzipSync(Buffer.from(json));
  await writeFile(outputPath, compressed);

  const sizeMb = (compressed.length / 1024 / 1024).toFixed(1);
  console.log(
    `[write-output] Wrote ${output.meta.recordCount} records to ${outputPath} (${sizeMb} MB gzipped)`,
  );

  await updateReadmeDate(outputPath, nasrCycleDate);
  await writeDatasetMeta(outputPath, {
    constName: 'usBundledAirportsProperties',
    typeName: 'AirportDatasetProperties',
    datasetName: 'airport',
    generatedBy: 'tools/build-airport-data',
    properties: output.meta,
  });
}
