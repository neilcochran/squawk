...
- Tools workspaces declare `"lint": "tsc --noEmit && eslint src"` so `turbo run lint` covers them. Everything private - the root and every `tools/*`, which is also what `scripts/*` resolves against - declares the development floor, currently `"engines": { "node": ">=24" }`, matching [.nvmrc](.nvmrc). Published packages keep their own lower floor. See [Node versions](ARCHITECTURE.md#node-versions) for which floor moves when.
...
Each published `@squawk/*` package that exposes an importable entrypoint commits a Markdown snapshot of its public API surface at `packages/libs/<pkg>/api/<pkg>.api.md`. The snapshot is generated by [@microsoft/api-extractor](https://api-extractor.com/) from the package's compiled `.d.ts` output. A published package that ships only a `bin` (`@squawk/adsbtop`, `@squawk/adsbscope`) has no importable surface to track and defines no `api:check` script; its CLI behaviour is covered by tests instead. The CI `api:check` gate fails when a code change widens, narrows, or otherwise alters the public surface without a matching update to the committed report, so accidental surface changes (e.g. an `export *` that picks up an internal helper) show up as a reviewable diff instead of slipping through.

When `api:check` fails on a PR, run `npm run api:report` locally, review the regenerated reports, and commit them alongside the code change. Each baseline includes any pre-existing `ae-*` or `tsdoc-*` warnings inline (via `addToApiReportFile: true`) so they remain visible without failing the gate.
...
7. A library may depend on another `@squawk/*` library - including one domain library on another (e.g. `@squawk/flightplan` on `@squawk/flight-math`) - when that is the clearest design and avoids duplicating logic. Keep such dependencies purposeful rather than incidental, and avoid import cycles (`import/no-cycle` is enforced in lint). The foundational libraries (`@squawk/types`, `@squawk/units`, `@squawk/geo`, `@squawk/search`) remain the common substrate most packages build on.
